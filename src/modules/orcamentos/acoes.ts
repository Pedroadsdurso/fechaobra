...
export async function salvarRascunho(rascunho: RascunhoOrcamento): Promise<ResultadoSalvar> {
  ...
  revalidatePath('/painel/orcamentos')
  return { ok: true, dataValidade: atualizado?.data_validade ?? undefined, idsItens }
}
 ...
 */
export async function duplicarOrcamento(id: string): Promise<{ ok: boolean; id?: string; erro?: string }> {
  const { supabase, user } = await exigirUsuario()

  const { data: origem } = await supabase
    .from('orcamentos')
    .select('titulo, tipo_servico, local_servico, validade_dias, prazo_execucao, texto_escopo, texto_exclusoes, texto_garantia, texto_condicoes_pagamento, observacoes')
    .eq('id', id)
    .maybeSingle()
  ...
  if (niveisUsados.size > 1) {
    ...
    if (pacotes && pacotes.length > 0) {
      ...
      await supabase
        .from('orcamento_pacotes')
        .insert(pacotes.map((p) => ({ ...p, orcamento_id: novo.id, destaque: false })))
